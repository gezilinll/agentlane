...
- Agentlane 统一消费标准化后的 Device、Runtime、ManagedAgent、ChannelBinding、RuntimeEndpoint、DeviceConnection 对象。
...
- RuntimeEndpoint：Runtime 暴露的有意义入口，例如 OpenClaw gateway、浏览器调试端口、本地 HTTP bridge 或远端平台 server url。
...
Agentlane 只展示对识别设备、排查连接和理解运行入口有用的字段。

- 设备身份：优先展示 Agentlane `deviceId` 和用户可读 `deviceName`，hostname 作为系统上报信息保留。
- 平台身份：可以记录 Slock machine/server 标识、Multica daemon id、OpenClaw gateway id 等平台注册身份。
- IP 地址：只保留主连接 IP 或后端观测到的公网出口 IP；不默认列出所有网卡地址。
- MAC 地址：默认不展示。确有设备识别需求时，只允许采集主网卡 MAC 并脱敏展示。
- 端口：挂在 RuntimeEndpoint 上，不作为 Device 的通用字段。设备侧采用 outbound 连接时，不要求开放入站端口。

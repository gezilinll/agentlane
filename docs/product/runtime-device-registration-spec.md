...
本地开发阶段同时保留 Vite dev server 中间件和独立 backend 服务的最小 API 闭环：

- `POST /api/device-snapshots`：Collector 上报一次 `RuntimeInventorySnapshot`，服务端做最小结构校验并保存为最新快照。
- `GET /api/runtime-fleet`：Runtime Fleet 页面优先读取的正式查询 API。
- `GET /api/runtime-inventory/latest`：兼容期 latest 快照 API；没有设备上报时返回 `404`，前端回退到 fixture。
...
- `GET /api/runtime-work-items`：Runs / Work Board 页面优先读取的正式查询 API。
- `GET /api/runtime-work-state/latest`：兼容期最新工作态快照 API；没有设备上报时返回 `404`，前端回退到明确标识的 fixture。

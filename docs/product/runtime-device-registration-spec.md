...
- Collector 以只读方式识别本机 runtime、agent、channel binding、最近同步、统一运行统计和健康状态。
- Collector 主动向 Agentlane 上报 snapshot；在没有服务端域名的开发期，可以本地路径安装并以 `--once` 输出 snapshot。
- 本地开发后端接收最新 snapshot，维护设备连接状态，并提供 Runtime Fleet 页面读取的 latest inventory API。
...
Adapter 拿不到某个统计字段时不伪造数据，由 UI 展示为不支持采集。
...
- `POST /api/devices/:deviceId/refresh`：Runtime Fleet 请求后端通过控制面向在线设备下发 `inventory.refresh`。
...
- `POST /api/runtime-work-state-snapshots`：Collector 上报一次 `RuntimeWorkStateSnapshot`，服务端做最小结构校验并保存为最新工作态快照。
- `GET /api/runtime-work-state/latest`：Runs / Work Board 页面读取最新工作态快照；没有设备上报时返回 `404`，前端回退到明确标识的 fixture。

默认 inventory 存储路径是 `.agentlane/runtime-inventory/latest.json`，可通过 `AGENTLANE_RUNTIME_INVENTORY_PATH` 覆盖。默认 work-state 存储路径是 `.agentlane/runtime-work-state/latest.json`，可通过 `AGENTLANE_RUNTIME_WORK_STATE_PATH` 覆盖。这些存储只代表本地开发期 latest snapshot，不是生产 Registry、审计日志或多设备数据库。
...
- `inventory.refresh`：后端向设备下发刷新 snapshot 命令。
...
- Slock 本地 workspace 只能证明 Agent 被识别，不能证明正在执行；没有运行中任务证据时 Agent 状态为 `unknown`。
...
- 看板中的 `in_progress` 不等于 runtime 正在执行，不能直接映射为 `RuntimeExecution.status=running`。
...
- 能通过 Runtime harness 验证 adapter 把平台字段转换成 Agentlane 状态、`lastSeenAt` 和运行统计语义。
- 能通过前端 harness 验证刷新按钮向后端请求远程刷新，并展示成功或失败状态。
- 能通过前端 harness 验证页面自动读取 latest snapshot，且不展示原始 UTC ISO 时间。
- 能通过 Runtime Work Board harness 验证页面只消费统一 WorkStage / confidence，不直接解释平台原始状态。
- 能通过 Runtime Work Board harness 验证 Slock `in_progress` 显示为 `processing + partial`，OpenClaw 成功 execution 直接进入 closed，失败 execution 进入 attention。

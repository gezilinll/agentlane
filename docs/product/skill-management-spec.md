...
- `files`：已规范化的文件快照，至少包含文本 `SKILL.md`。文本文件可以包含 `content`；图片、压缩包、图标等二进制资源只记录 `path`、`contentHash`、`sizeBytes` 和 `contentType: "binary"`，不得把原始二进制内容写入 JSON 或日志。
- `lastSeenAt`：最近一次发现时间。
...
- 用户点击“提升为组织 Skill”时，后端必须把发现结果中的可编辑文本文件复制成新的组织 Skill 和 Skill Version，来源类型为 `device_discovery`。二进制资源在当前页面不支持编辑和重新打包，提升时不复制；后续如果引入对象存储或资产编辑器，再把二进制资源纳入组织 Skill 文件副本。
...
- `GET /api/skill-discoveries`：按组织和可选 `deviceId`、`runtimeId`、`agentId`、`targetType`、`targetId` 读取当前 collector 发现的目标本地 Skill。Runtime Fleet 深链到 `/skills?targetType=agent&targetId=...` 时，Skill 管理页面使用 `targetType + targetId` 读取该目标的本地发现 Skill。
...
当前 Skill 管理页面从 Runtime Fleet 查询结果中派生可分配目标，并在用户选择目标后调用 Skill target HTTP API 展示目标 Skill Set 与安装状态，同时调用 Skill discovery API 展示该目标本地已发现的 Skill。Target Skill Set 代表已经通过 Lorume Assignment 显式下发或待同步的 Skill；目标本地发现 Skill 代表设备当前目录中已有、尚未提升为组织资产的 Skill。页面不得在前端自行拼接生效规则，也不得调用不存在的 target support 原因 API。
当前 Skill 管理页面可以对已批准、已同步、失败或不支持的 Assignment 触发“同步到目标”；该动作只创建 Operation，不直接写设备。
Runtime Fleet 的 Agent 详情可以通过 `/skills?targetType=agent&targetId=...` 深链打开 Skill 管理，并预选目标 Skill Set 与目标本地发现 Skill。`targetId` 必须 URL 编码，页面需要在目标列表异步加载前保留该选择，不能因为 target ID 暂时不在 options 中而丢失。
...
- 展示设备发现到的本地 Skill，并支持把发现结果提升为独立组织 Skill。用户选择 Device、Runtime 或 Agent 目标后，页面必须优先展示该目标范围内发现到的本地 Skill；没有选择目标时才展示组织内可见的全局发现列表。
...
- 选择目标后展示后端按 `targetType + targetId` 查询到的目标本地发现 Skill，包含 Skill 名称、来源平台、目标类型和发现时间，并允许提升为组织 Skill。
...
- Collector 能从配置的 Skill discovery 目标目录发现包含 `SKILL.md` 的包，并随 Runtime inventory 上报。OpenClaw 默认发现路径必须覆盖当前 workspace 级目录 `~/.openclaw/workspace/skills`，避免真实 OpenClaw Agent Skill 被漏采。
- 发现结果提升为组织 Skill 会复制文本文件内容并重新校验；二进制资源只保留发现元数据，不进入当前组织 Skill 副本；来源目标后续删除不会影响组织 Skill。
...
- Skill 管理覆盖设备发现 Skill 列表和提升为组织 Skill；Runtime Fleet 深链进来时，页面必须展示该目标本地发现 Skill；提升调用正式 `POST /api/skill-discoveries/:discoveryId/promote`，不在前端拼接 Skill 内容。
...
- 选择目标后调用 `GET /api/skill-targets/:targetType/:targetId/skill-set` 和 `GET /api/skill-discoveries?targetType=...&targetId=...`，页面只展示后端解析出的 Target Skill Set 与该目标本地发现 Skill，不把组织 Skill 库里的未分配 Skill 当作目标已生效 Skill。
- 最新版本未发布时不能创建分配。

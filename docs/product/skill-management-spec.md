...
- 当前确定性写回根目录由 collector 配置决定；没有 runtime 专属 adapter 时默认写入 collector 的 Skill sync root，不由 UI 拼接本地路径。
...
- Collector 能把 `skill.sync` 文件 payload 写入配置的 Skill sync root，拒绝目录逃逸、文件大小不匹配和 hash 不匹配。

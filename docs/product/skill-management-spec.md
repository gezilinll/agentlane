...
Skill 是组织级能力资产。它拥有名称、描述、owner、生命周期、权限和版本列表。
...
Skill Assignment 表示“组织 Skill 的某个版本被分配到某个目标”。它不是安装记录，也不等于目标上已经存在该 Skill。
...
Skill Sync Job 是一次确定性下发、删除或校验操作的审计记录。它记录操作结果，不承担长期安装状态。
...
Skill 可以分配到 Device、Runtime 或 Agent。生效规则由查询层计算，不在前端拼接。
...
Skill 导入、校验、发布、审核、分配、下发和归档产生的通知由 [Notification Spec](./notification-spec.md) 统一管理。
...
- `POST /api/skill-assignments`：创建分配或创建分配审核。
- `POST /api/skill-assignments/:assignmentId/sync`：下发分配或创建同步审核。

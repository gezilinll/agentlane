...
本规格定义 Agentlane 的公共通知机制。通知用于把需要关注、需要处理、已完成或已失败的异步事件传达给相关人，覆盖 Skill、Agent 迁移、设备采集、审核、组织邀请和系统健康等模块。
...
- `GET /api/notifications/:threadId`：读取 Thread 详情和近期 Event。
- `POST /api/notifications/:threadId/read`：标记已读。
- `POST /api/notifications/:threadId/mute`：静音 Thread。
- `GET /api/notification-preferences`：读取通知偏好。

...
- 使用奶油底、黑色硬边、高饱和强调色和像素化阴影建立辨识度。
- 视觉 token 必须集中定义，页面和组件通过变量复用，不在业务组件里散落硬编码色值。
- 品牌标题、页面大标题、短按钮、状态 badge、命令式短标签优先使用 Fusion Pixel。
- 身份入口页可以整体强化像素字体以贴近 Cream Arcade 品牌感；Console 的正文、长说明、列表和表格需要优先保证扫描效率，可以使用 Noto Sans SC 或系统 sans-serif fallback。
- ID、命令、时间戳、token prefix 等技术值使用 monospace。
- 登录、组织邀请等身份页面和 Console 页面共用同一套视觉 token。身份页承载更强的像素游戏感，Console 页面保留更高信息密度，但不能脱离 Cream Arcade 的硬边、奶油底、像素阴影和高饱和强调色。
- 像素装饰只用于空白节奏和品牌氛围，不承载业务含义，不遮挡信息，不制造滚动或响应式问题。
- 产品内图标必须通过共享像素图标组件进入页面，避免混用 lucide、临时 CSS 手绘、文本符号和不同线宽的 SVG。登录页、Console 导航、工具栏、状态概览、输入框和背景装饰都遵循同一套像素图标语法。
- 像素面板、按钮、输入框、badge、侧栏和装饰元素必须优先复用 `src/ui/tokens.css` 与 `src/ui/` 基础组件；如果视觉效果需要调整，先调整 token 或共享组件，再落到业务页面。
...
- People & Access 负责成员、角色、邀请和设备 token 管理。

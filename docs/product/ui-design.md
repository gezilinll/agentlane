...
Purpose:
把 Skill 作为组织级一等资产管理，支持导入、查看、编辑、校验、发布、审核、分配到 Device / Runtime / Agent、下发校验、归档和删除。详细规则以 [Skill Management Spec](./skill-management-spec.md) 为准。
...
- 打开 `SKILL.md`、文件树、版本、校验结果、权限和目标分配。
- 导入 Markdown、ZIP、GitHub URL 或 Marketplace 来源。
- 编辑草稿文件并发布不可变版本。
- 对高风险或权限不足的发布、分配、下发和归档进入审核。
- 选择 Device、Runtime 或 Agent 作为目标并执行确定性下发。
- 查看下发结果、不支持原因和最近同步状态。
- 归档 Skill 或删除未发布草稿 / 未使用版本。

Data objects:
Skill、SkillVersion、SkillFile、SkillAssignment、SkillSyncJob、ResourcePermission、ApprovalRequest、NotificationThread。
...
- 编辑草稿文件后提交发布。
- 通过包校验、依赖提示和权限检查后创建不可变版本。
- 需要审核时创建 Approval Request。
- 审核通过后分配并下发到选定 Device、Runtime 或 Agent。
- 通过 adapter / collector / CLI / 文件同步校验目标结果。
- 出问题时停用分配、重试同步、归档或删除未使用版本。
...
- 已实现页面是 Catalog、Runtime Fleet 和 Runs / Work Board。
- Skill Registry & Editor、Agent 迁移、通知中心、Agent Studio、Workflow Studio、Governance Center、Integrations & Resources 等页面进入实现前，必须先补齐对应 spec、对象模型、权限边界和 harness。

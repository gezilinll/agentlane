...
Agentlane 的长期 UI 方向是 **Cream Arcade**：像素风、极客工具感和克制游戏感。它服务于“团队操作台”的产品定位，不应该把 Console 做成营销页或玩具界面。
...
Agentlane 当前采用桌面 Web Console。当前可操作导航只暴露已经落地并可验收的页面：
...
- `/` 是公开主页，用来解释 Agentlane 的控制面定位和当前已接入能力。
- `/login` 是邮箱验证码登录入口。
- `/invite/:token` 是组织邀请入口，用户登录被邀请邮箱后加入组织。
- `/catalog`、`/runtime`、`/runs` 是当前 Console 深链路由，未登录时需要先完成登录。
- 未登录用户进入邮箱验证码登录。
- 登录后如果没有组织，需要创建组织或通过邀请链接加入组织。
- 成员、角色、邀请和设备 token 管理先由组织/auth 后端能力承载；正式 People & Access 页面在实现前不出现在导航。
...
- 管理对象目录：登录 -> Catalog -> 查看正式对象、owner 槽位、生命周期和依赖关系。

长期用户动线：

- 运营者：Command Center -> Runs / Run Trace -> Worker Fleet / Governance Center
- Agent/Workflow 创建者：Catalog -> Agent Studio / Workflow Studio / Skill Registry -> Dry Run -> Publish -> Runs
...
Exit points:
Agent Studio、Workflow Studio、Skill Registry、Integrations & Resources、Governance Center。
...
Exit points:
Runs / Run Trace、Catalog、Governance Center、Skill Registry。
...
Exit points:
Runs / Run Trace、Governance Center、Agent Studio、Skill Registry。

## 05. Skill Registry

![Skill Registry](../../assets/product-ui/05-skill-registry-editor.png)
...
- 从失败节点跳转到 Worker Fleet、Skill Registry 或 Governance。
...
Exit points:
Worker Fleet、Skill Registry、Governance Center、Output、Decision Record。
...
Entry points:
Catalog、Agent Studio、Workflow Studio、Skill Registry、Governance 风险告警。
...
- 从 Skill Registry 打开 Skill Detail。
...
- 已实现页面是 Catalog、Runtime Fleet、Runs / Work Board 和 Skill Registry。
- Agent 迁移、通知中心、Agent Studio、Workflow Studio、Governance Center、Integrations & Resources 等页面进入实现前，必须先补齐对应 spec、对象模型、权限边界和 harness。
- Integrations & Resources 接入 OpenClaw、Multica、Slock、Codex、DingTalk、Telegram、Slack、BI、星图、SLS、GitLab、Aetheris CLI 等资源时，平台差异必须由 adapter 转换为 Agentlane 语义。

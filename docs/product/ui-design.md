...
- 品牌标题、页面大标题、短按钮、状态 badge、命令式短标签可以使用 Fusion Pixel。
- 正文、表单、长说明、列表和表格使用 Noto Sans SC 或系统 sans-serif fallback，避免像素字体影响阅读。
- ID、命令、时间戳、token prefix 等技术值使用 monospace。
- 登录、组织邀请等身份页面先承载新视觉语言；Console 页面按相同 token 逐步统一，但不改变 Runtime / Agent / WorkItem 的产品语义。

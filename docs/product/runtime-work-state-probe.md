...
前端看板必须服务“用户能找到自己发给 Agent 的那条任务”这个目标。能够进入 Runs / Work Board 主泳道的卡片必须有业务上下文，至少来自 `RuntimeWorkItem` 或等价上游任务对象。裸 `RuntimeExecution` 只能作为关联证据或后台运行记录，不能单独生成用户可见任务卡。

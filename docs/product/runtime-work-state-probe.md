...
- OpenClaw 的 channel 不能在产品层硬编码为 DingTalk。v1 已验证 DingTalk 的本地 state 与 session key；后续 Telegram、Slack 等 OpenClaw channel 也要在 OpenClaw adapter 内转成 Lorume 的 `Channel` 与 `会话/群组`，前端只消费归一化字段。

...
- 将 Runs / Runtime Fleet 的正式数据读取从“前端拉 latest snapshot 后本地筛选”逐步迁移为“后端查询、前端展示”。
...
- Work state 不长期依赖“全量历史覆盖 latest”。后端按稳定 ID upsert 工作项、会话和执行记录。

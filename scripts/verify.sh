...
for path in "${required_paths[@]}"; do
  if [[ ! -e "$path" ]]; then
    echo "verify: missing required path: $path" >&2
    exit 1
  fi
done

if ! command -v python3 >/dev/null 2>&1; then
  echo "verify: python3 is required for Markdown link checks" >&2
  exit 1
fi
...
if problems:
    print("verify: Markdown link check failed", file=sys.stderr)
